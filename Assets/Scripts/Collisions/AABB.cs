...
public struct AABB
{
	...
	public void Draw(Color color, float width = 0.05f)
	{
		...
		Lines.Instance.AddLine(new Vector2(max.x, max.y), new Vector2(max.x, max.y), color, width);
		Lines.Instance.AddLine(new Vector2(min.x, min.y), new Vector2(max.x, min.y), color, width);
		// include the other three lines of the AABB draw, the one above draws the top line}
	}

	public void SetMinMax(Vector2 min, Vector2 max)
	{
		size = (max - min); center = min + extents;
	}
